...
import { animate, circOut, scroll, stagger } from "motion";
...
state.featureSideEffects = {
  ...
  3: {
    ...
    enter: () => {
      ...
      animate(topItems, { opacity: [0, 1], x: [500, 0] }, {
        duration: 0.35,
        ease: `easeOut`,
        delay: stagger(0.1),
      });
  ...
  11: {
    ...
    enter: () => {
      playTopForgotten();
      let forgottenFavoriteTracks = document.querySelectorAll(
        `#top-forgotten-main-feature li`,
      );
      animate(forgottenFavoriteTracks, { opacity: [0, 1], x: [500, 0] }, {
        duration: 0.35,
        ease: `easeOut`,
        delay: stagger(0.1),
      });
  ...
  13: {
    ...
    enter: () => {
      if (state.extraFeatures.leastSkippedTracks) {
        ...
        let topItems = document.querySelectorAll(
          `#least-skipped-tracks-main-feature li`,
        );
        animate(topItems, { opacity: [0, 1], x: [500, 0] }, {
          duration: 0.35,
          ease: `easeOut`,
  ...
  14: {
    ...
    enter: () => {
      if (state.extraFeatures.mostSkippedTracks) {
        ...
        let topItems = document.querySelectorAll(
          `#most-skipped-tracks-main-feature li`,
        );
        animate(topItems, { opacity: [0, 1], x: [500, 0] }, {
          duration: 0.35,
          ease: `easeOut`,
...
state.features = [
    ...
    html`
      <div class="p-4">
        <div class="mt-6 w-full flex flex-col items-center mb-16">
          <img class="h-40" src="${() =>
            state.settings.darkMode
              ...
              : "/media/banner-light.svg"}" alt="Jellyfin Rewind Logo">
        </div>

        <h2
          class="text-[1.65rem] leading-8 text-center mt-16 font-semibold text-gray-800 dark:text-gray-200"
        >
          Welcome to<br>Jellyfin Rewind ${() => state.rewindReport?.year}!
        </h2>
        ...
        >
          ...
          <p class="">
            If you have any questions or feedback, please reach out to me on <a
        ...
        <div>
          <button
            ...
            @click="${stopPropagation(() => showOverlayDataAccuracy())}"
          >
            Accuracy Disclaimer
          </button>
        </div>
    ...
    html`
      <div class="text-center">
        <h2 class="text-2xl font-medium mt-5">
          Your Total Playtime<br>of ${() => state.rewindReport?.year}<span
        ...
        <div class="mt-10 -rotate-6 font-quicksand text-sky-500 text-4xl">
          <span class="font-quicksand-bold">${() =>
            showAsNumber(
              state.rewindReport.generalStats
                .totalPlaybackDurationMinutes[state.settings.dataSource]
                .toFixed(0),
            )}</span> min
        </div>
        ...
        <div class="mt-12 w-full flex flex-col items-center gap-0.5 text-sm">
          <div><span class="font-semibold">${() =>
            showAsNumber(
              state.rewindReport.generalStats?.[`totalPlays`]
                ?.[state.settings.dataSource],
            )}</span> total streams.</div>
          <div><span class="font-semibold">${() =>
            showAsNumber(
              state.rewindReport.generalStats?.[`uniqueTracksPlayed`],
            )}</span> unique tracks.</div>
          <div><span class="font-semibold">${() =>
            showAsNumber(
              state.rewindReport.generalStats?.[`uniqueArtistsPlayed`],
            )}</span> unique artists.</div>
          <div><span class="font-semibold">${() =>
            showAsNumber(
              state.rewindReport.generalStats?.[`uniqueAlbumsPlayed`],
            )}</span> unique albums.</div>
        </div>

        <div class="absolute bottom-20 w-full h-2/5 px-8">
          <canvas id="playtime-by-month-chart" class="${() =>
            state.extraFeatures.totalPlaytimeGraph
              ? ``
              : `opacity-30`}"></canvas>
          ${() =>
            state.extraFeatures.totalPlaytimeGraph
              ...
              : html`
                <div
                  class="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center gap-12"
                >
                  <span class="text-4xl rotate-12 text-[#00A4DC] tracking-wider font-semibold"
                  >Unavailable</span>
                  <button
                    @click="${stopPropagation(() =>
                      showOverlayFeatureUnavailableMissingPlaybackReporting()
                    )}"
  ...
  buildFeature(
    ...
    html`
      <div class="text-center text-white">
        <h2 class="text-2xl mt-5">Your Top Track<br>of ${() =>
          state.rewindReport?.year}:</h2>
            ...
            <div class="-rotate-6 -ml-10 mt-10 text-4xl font-semibold">
              <div class="">${() =>
                state.settings.useAlbumArtists
                  ? state.rewindReport.tracks?.[state.settings.rankingMetric]
                    ?.[0].albumBaseInfo.albumArtistBaseInfo.name
                  : state.rewindReport.tracks?.[state.settings.rankingMetric]
                    ?.[0].artistsBaseInfo
                    .reduce(
                      (acc, cur, index) =>
              ...
              <div class="mt-8 ml-10">${() =>
                state.rewindReport.tracks?.[state.settings.rankingMetric]?.[0]
                  ?.name}</div>
            </div>
          </div>
        </div>
        ...
        >
          <div>Streamed <span class="font-semibold">${() =>
            showAsNumber(
              state.rewindReport.tracks?.[state.settings.rankingMetric]?.[0]
                ?.playCount[state.settings.dataSource],
            )}</span> times.</div>
          <div>
            Listened for <span class="font-semibold">${() =>
              showAsNumber(
                state.rewindReport.tracks?.[state.settings.rankingMetric]?.[0]
                  ?.totalPlayDuration[state.settings.dataSource]?.toFixed(0),
              )}</span> minutes.
          </div>
        </div>
      </div>
  ...
  buildFeature(
    ...
    html`
      <div class="text-center">
        ...
        <ol id="top-tracks-main-feature" class="flex flex-col gap-2 p-6">
          ${() =>
            state.rewindReport.tracks?.[state.settings.rankingMetric]?.slice(
              0,
              5,
            ).map((track, index) =>
              html`
                ...
                >
                  ...
                  >
                    <img id="${() =>
                      `top-tracks-image-${index}`}" class="w-full h-full" />
                    <div
                  ...
                  >
                    <div class="flex flex-col gap-0.25 items-start">
                      ...
                      <span class="text-sm ml-2 max-h-[2rem] text-ellipsis overflow-hidden"
                      >by ${() =>
                        state.settings.useAlbumArtists
                    ...
                    >
                      <div>
                        ...
                        >${() =>
                          showAsNumber(
                            track.playCount[state.settings.dataSource],
                          )}</span> streams
                      </div>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                      ...
                      <div>
                        ...
                        >${() =>
                          showAsNumber(
                            track.totalPlayDuration[state.settings.dataSource]
                              .toFixed(0),
                          )}</span> minutes
                      </div>
                    </div>
                  ...
                  >
                    <img id="${() =>
                      `top-tracks-background-image-${index}`}" class="w-full h-full" />
                  </div>
                </li>
              `.key(track.id)
                ...
                <div class="flex flex-col gap-1 w-full">
                  <div class="flex flex-col gap-0.25 items-start">
                    ...
                    <div class="ml-6 max-h-[2rem] text-xs">
                      by <span class="font-semibold text-ellipsis overflow-hidden"
                      >${() =>
                        state.settings.useAlbumArtists
                          ? track.albumBaseInfo.albumArtistBaseInfo.name
                          : track.artistsBaseInfo
                  ...
                  <!--
                    <div class="flex flex-row justify-start font-medium text-gray-800 gap-0.5 items-center text-xs">
                      <div><span class="font-semibold text-black">${showAsNumber(
                        track.playCount[state.settings.dataSource],
                      )}</span> streams</div>
          ...
          <div class="mt-16 -rotate-6 font-quicksand text-sky-500 text-8xl">
            <span class="font-quicksand-bold">${() =>
              state.extraFeatures.totalMusicDays
                ? showAsNumber(animators.totalMusicDays.toFixed(0))
                : `???`}</span>
          </div>

          <div class="mt-16 w-full px-10 flex flex-col items-center gap-6">
            <div class="font-semibold text-xl">
              That's on how many days you listened to music through Jellyfin this year.
            </div>
            ${() =>
              state.extraFeatures.totalMusicDays
                ? (
                  state.rewindReport.generalStats.totalMusicDays < 364
                    ? html`
                      <div class="font-semibold text-sm text-center">
                        What did you do on the ${(365 -
                          state.rewindReport.generalStats.totalMusicDays)
                          .toFixed(0)} missing days?!
                      </div>
                    `
                    : html`
                      <div class="font-semibold text-sm text-center">
                        Good news: next year you can get one additional day!
                      </div>
                    `
                )
                : null}
              ...
              ? html`
                <div class="mt-24 w-full px-10 flex flex-col items-center gap-3">
                  <div class="font-semibold text-xl">
                    On those ${() =>
                      state.rewindReport.generalStats
                        .totalMusicDays} days,<br>you listened to <span
                      class="text-3xl text-sky-500 font-quicksand"
                    >${() =>
                      showAsNumber(
                        state.rewindReport?.generalStats?.minutesPerDay?.mean
                          .toFixed(0),
                      )}</span> minutes per day on average.
                  </div>
                  <div class="font-semibold text-xl">
                    ...
                    hours or <span class="text-2xl text-sky-500 font-quicksand"
                    >${() =>
                      showAsNumber(
                        (state.rewindReport?.generalStats?.minutesPerDay?.mean /
                          60.0 / 24.0 * 100.0).toFixed(1),
                      )}%</span> of a day.
                  </div>
                  <div class="font-semibold text-sm px-8 pt-6">
                    ...
                    >${() =>
                      showAsNumber(
                        state.rewindReport?.generalStats?.minutesPerDay?.median
                          .toFixed(1),
    ...
    buildFeature(
      ...
      html`
        <div class="text-center text-white">
          <h2 class="text-2xl mt-5">Your Top Artist<br>of ${() =>
            state.rewindReport?.year}:</h2>
          <div class="flex mt-10 flex-col">
            <img
              id="top-artist-image"
            ...
            <div class="px-4 py-4 overflow-hidden whitespace-wrap">
              <div class="-rotate-6 mt-16 text-5xl font-semibold">
                <div class="">${() =>
                  state.rewindReport.artists?.[state.settings.rankingMetric]
                    ?.[0]?.name}</div>
              </div>
            </div>
          </div>
          ...
          >
            <div>Streamed <span class="font-semibold">${() =>
              showAsNumber(
                state.rewindReport.artists?.[state.settings.rankingMetric]?.[0]
                  ?.playCount?.[state.settings.dataSource],
              )}</span> times.</div>
            <div>
              Listened to <span class="font-semibold">${() =>
                showAsNumber(
                  state.rewindReport.artists?.[state.settings.rankingMetric]
                    ?.[0]?.uniquePlayedTracks?.[state.settings.dataSource],
                )}</span> unique tracks
              <br>for <span class="font-semibold">${() =>
                showAsNumber(
                  state.rewindReport.artists?.[state.settings.rankingMetric]
                    ?.[0]?.totalPlayDuration[state.settings.dataSource].toFixed(
                      0,
                    ),
                )}</span> minutes.
            </div>
    ...
    buildFeature(
      ...
      html`
        <div class="text-center">
          ...
          <ol id="top-artists-main-feature" class="flex flex-col gap-2 p-6">
            ${() =>
              state.rewindReport.artists?.[state.settings.rankingMetric]?.slice(
                0,
                5,
              ).map((artist, index) =>
                html`
                  ...
                  >
                    ...
                    >
                      <img id="${() =>
                        `top-artists-image-${index}`}" class="w-full h-full" />
                      <div
                    ...
                    >
                      ...
                      >
                        <div>
                          ...
                          >${() =>
                            showAsNumber(
                              artist.playCount[state.settings.dataSource],
                            )}</span> streams
                        </div>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div>
                          <span class="font-semibold text-black dark:text-white"
                          >${() =>
                            showAsNumber(artist.uniqueTracks)}</span> tracks
                        </div>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          class="w-3 h-3 stroke-2 icon icon-tabler icon-tabler-point"
                        ...
                        <div>
                          ...
                          >${() =>
                            showAsNumber(
                              artist
                                .totalPlayDuration[state.settings.dataSource]
                                .toFixed(0),
                            )}</span> min
                        </div>
                      </div>
                    ...
                    >
                      <img id="${() =>
                        `top-artists-background-image-${index}`}" class="w-full h-full" />
                    </div>
                  </li>
                `.key(artist.id)
                // .key((Math.random() * 100000).toString(16)) // assign a random key to force re-rendering of the list (and thus the indices)
                  ...
                  <div class="flex flex-col gap-1 w-full">
                    ...
                    <!--
                      <div class="flex flex-row justify-start font-medium text-gray-800 gap-0.5 items-center text-xs">
                        ...
                        <div><span class="font-semibold text-black">${showAsNumber(
                          artist.totalPlayDuration[state.settings.dataSource]
                            .toFixed(0),
                        )}</span> minutes</div>
                      </div>
                      -->
          ...
          <div class="text-center">
            ...
            <div class="mt-12 w-full px-6 flex flex-col items-center gap-2">
              <div class="font-semibold text-xl">
                ...
                >${() =>
                  showAsNumber(
                    (state.rewindReport?.libraryStats?.totalRuntime / 60.0 /
                      60.0).toFixed(0),
                  )}</span> hours.
              </div>
              <div class="font-semibold text-xl">
            ...
            <div class="mt-16 w-full px-10 flex flex-col items-center gap-3">
              <div class="font-semibold text-xl">
                <span class="text-3xl text-sky-500 font-quicksand">${() =>
                  showAsNumber(
                    state.rewindReport?.libraryStats?.tracks?.favorite,
                  )}</span>
                favorite tracks.
              </div>
              <div class="font-semibold text-xl">
                <span class="text-3xl text-sky-500 font-quicksand">${() =>
                  showAsNumber(
                    state.rewindReport?.libraryStats?.tracks?.total,
                  )}</span>
                unique tracks.
              </div>
              <div class="font-semibold text-xl">
                <span class="text-3xl text-sky-500 font-quicksand">${() =>
                  showAsNumber(
                    state.rewindReport?.libraryStats?.albums?.total,
                  )}</span>
                albums.
              </div>
              <div class="font-semibold text-xl">
                <span class="text-3xl text-sky-500 font-quicksand">${() =>
                  showAsNumber(
                    state.rewindReport?.libraryStats?.artists?.total,
                  )}</span>
                artists.
              </div>
              <div class="font-semibold text-xl mt-4">Impressive.</div>
            </div>

            <div class="mt-16 w-full px-10 flex flex-col items-center gap-2">
              <div class="font-semibold text-xl">
                Average track length: <span class="text-2xl text-sky-500 font-quicksand"
                >${() =>
                  showAsNumber(
                    state.rewindReport?.libraryStats?.trackLength?.mean.toFixed(
                      0,
                    ),
                  )}</span> seconds.
              </div>
              <div class="font-semibold -mt-1">
                (Median: <span class="text-sky-500 font-quicksand">${() =>
                  showAsNumber(
                    state.rewindReport?.libraryStats?.trackLength?.median
                      .toFixed(0),
                  )}</span>)
              </div>
              <div class="font-semibold text-xl">
                ...
                >${() =>
                  showAsNumber(
                    state.rewindReport?.libraryStats?.trackLength?.min.toFixed(
                      0,
                    ),
                  )}</span> seconds.
              </div>
              <div class="font-semibold text-xl">
                ...
                >${() =>
                  showAsNumber(
                    state.rewindReport?.libraryStats?.trackLength?.max.toFixed(
                      0,
                    ),
                  )}</span> seconds.
              </div>
              <div class="font-semibold text-xl mt-4">Surprised?</div>
            </div>
          </div>
        `,
        `bg-[#00A4DC]/10 dark:bg-[#000B25] dark:text-white`,
      ...
      buildFeature(
        ...
        html`
          <div class="text-center text-white">
            <h2 class="text-2xl mt-5">Your Top Album<br>of ${() =>
              state.rewindReport?.year}:</h2>
            <div class="flex mt-10 flex-col items-center">
              ...
              <div class="px-4 py-4 overflow-hidden whitespace-wrap">
                <div class="-rotate-6 mt-10 text-4xl font-semibold">
                  <div class="-ml-4">${() =>
                    state.rewindReport.albums?.[state.settings.rankingMetric]
                      ?.[0].name}</div>
                  <div class="ml-4 mt-8 max-h-[3.5em]">by ${() =>
                    state.settings.useAlbumArtists
                      ? state.rewindReport.albums
                        ?.[state.settings.rankingMetric]?.[0]?.albumArtist.name
                      : state.rewindReport.albums
                        ?.[state.settings.rankingMetric]?.[0]?.artists
                        .reduce(
                          (acc, cur, index) =>
                            index > 0 ? `${acc} & ${cur.name}` : cur.name,
                          ``,
            ...
            >
              <div>Streamed <span class="font-semibold">${() =>
                showAsNumber(
                  state.rewindReport.albums?.[state.settings.rankingMetric]?.[0]
                    ?.playCount[state.settings.dataSource],
                )}</span> times.</div>
              <div>
                Listened for <span class="font-semibold">${() =>
                  showAsNumber(
                    state.rewindReport.albums?.[state.settings.rankingMetric]
                      ?.[0]?.totalPlayDuration[state.settings.dataSource]
                      ?.toFixed(0),
                  )}</span> minutes.
              </div>
            </div>
      ...
      buildFeature(
        ...
        html`
                  ...
                  ?.slice(0, 5).map((album, index) =>
                    html`
                      ...
                      >
                        ...
                        >
                          <img id="${() =>
                            `top-albums-image-${index}`}" class="w-full h-full" />
                          <div
                        ...
                        >
                          ...
                          >
                            <div>
                              ...
                              >${() =>
                                showAsNumber(
                                  album.playCount[state.settings.dataSource],
                                )}</span> streams
                            </div>
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                        ...
                        >
                          <img id="${() =>
                            `top-albums-background-image-${index}`}" class="w-full h-full" />
                        </div>
                      </li>
                    `.key(album.id)
                    // .key((Math.random() * 100000).toString(16)) // assign a random key to force re-rendering of the list (and thus the indices)
          ...
          >
            ${() =>
              state.rewindReport.albums?.[state.settings.rankingMetric]?.slice(
                5,
                20,
              ).map((album, index) =>
                html`
                  <li class="relative overflow-hidden w-1/2 mx-auto pl-3">
                    <div class="flex flex-col gap-1 w-full">
                      ...
                      <!--
                        <div class="flex flex-row justify-start font-medium text-gray-800 gap-0.5 items-center text-xs">
                          <div><span class="font-semibold text-black">${album
                            .playCount[
                              state.settings.dataSource
                            ]}</span> streams</div>
                          <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 stroke-2 icon icon-tabler icon-tabler-point" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="text-center">
              ...
              <div class="mt-24 w-full px-6 flex flex-col items-center gap-2">
                <div class="font-semibold text-xl">
                  This year, you had <span
                    class="font-semibold text-3xl text-sky-500 font-quicksand"
                  >${() =>
                    state.extraFeatures.listeningActivityDifference
                      ? showAsNumber(
                        Math.abs(
                          state.rewindReport?.featureDelta
                            ?.listeningActivityDifference
                            ?.totalPlays[state.settings.dataSource],
                        ).toFixed(0),
                      )
                      : `???`}</span> ${() =>
                    (!state.extraFeatures.listeningActivityDifference ||
                        state.rewindReport?.featureDelta
                            ?.listeningActivityDifference
                            ?.totalPlays[state.settings.dataSource] >= 0)
                      ? `more`
                      : `less`} streams than in ${() =>
                    state.rewindReport?.featureDelta?.year ??
                      (state.rewindReport?.year - 1)}.
                </div>
              </div>
              ...
              <div class="mt-24 w-full px-6 flex flex-col items-center gap-2">
                <div class="font-semibold text-xl">
                  You ${() =>
                    (!state.extraFeatures.listeningActivityDifference ||
                        state.rewindReport?.featureDelta?.favoriteDifference >=
                          0)
                      ? `added`
                      : `removed`} <span
                    class="font-semibold text-3xl text-sky-500 font-quicksand"
                  ...
                  : html`
                    ...
                    >
                      ...
                      >
                        ...
                        <button
                          @click="${stopPropagation(() =>
                            showOverlayFeatureUnavailableMissingOldReport()
                          )}"
                          class="w-32 rounded-md flex flex-row items-center justify-around px-2 py-1 bg-white text-gray-900"
                        >
                          <svg
        ...
        buildFeature(
          ...
          html`
            <div class="text-center">
              ...
              >
                ${() =>
                  state.rewindReport.genres?.[state.settings.rankingMetric]
                      ...
                      html`
                        ...
                        >
                          <div class="flex flex-col gap-1 overflow-hidden h-full w-full rounded-md">
                            ...
                            >
                              <span class="font-semibold basext-xl">${() =>
                                index + 1}.</span>
                              <div class="flex flex-col gap-0.5 items-start">
                                <span class="font-quicksand-bold text-lg uppercase tracking-widest"
                                >${() => genre.name}</span>
                          ...
                          <div class="flex flex-col gap-0.25 items-start">
                            ...
                            >
                              <span class="font-semibold mr-2">${index + 1 +
                                5}.</span>
                              <span class="font-semibold leading-tight text-ellipsis overflow-hidden"
                              >${genre.name}</span>
                            </div>
                          </div>
                          <!--
                            <div class="flex flex-row justify-start font-medium text-gray-800 gap-0.5 items-center text-xs">
                              <div><span class="font-semibold text-black">${showAsNumber(
                                genre.playCount[state.settings.dataSource],
                              )}</span> streams</div>
                              <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 stroke-2 icon icon-tabler icon-tabler-point" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                <circle cx="12" cy="12" r="4"></circle>
          ...
          buildFeature(
            ...
            html`
              <div class="text-center">
                ...
                ${() =>
                  !state.extraFeatures.leastSkippedTracks
                    ? html`
                      <div
                    ...
                    : html`
                      <ol id="least-skipped-tracks-main-feature" class="flex flex-col gap-2 p-6">
                        ${() =>
                          state.rewindReport.tracks?.[`leastSkipped`]?.slice(
                            0,
                            5,
                          ).map((track, index) =>
                            html`
                              ...
                              >
                                ...
                                >
                                  <img id="${() =>
                                    `least-skipped-tracks-image-${index}`}" class="w-full h-full" />
                                  <div
                                    id="${() =>
                                      `least-skipped-tracks-visualizer-${index}`}"
                                    class="absolute top-0 left-0 w-full h-full grid place-content-center text-white bg-black/30 hidden"
                                  >
                                  </div>
                                </div>
                                ...
                                >
                                  <img id="${() =>
                                    `least-skipped-tracks-background-image-${index}`}" class="w-full h-full" />
                                </div>
                              </li>
                            `.key(track.id)
                            // .key((Math.random() * 100000).toString(16)) // assign a random key to force re-rendering of the list (and thus the indices)
              ...
              ${() =>
                !state.extraFeatures.leastSkippedTracks ? null : html`
                  ...
                  >
                    ${() =>
                      state.rewindReport.tracks?.[`leastSkipped`]?.slice(5, 20)
                        .map((track, index) =>
                          html`
                            <li class="relative overflow-hidden w-1/2 mx-auto pl-3">
                              <div class="flex flex-col gap-1 w-full">
            ...
            buildFeature(
              ...
              html`
                <div class="text-center">
                  ...
                  ${() =>
                    !state.extraFeatures.mostSkippedTracks
                      ? html`
                        <div
                          class="absolute top-0 left-0 w-full h-full flex flex-col items-center bg-black/40 justify-center gap-12"
                        >
                      ...
                      : html`
                        <ol id="most-skipped-tracks-main-feature" class="flex flex-col gap-2 p-6">
                          ${() =>
                            state.rewindReport.tracks?.[`mostSkipped`]?.slice(
                              0,
                              5,
                            ).map((track, index) =>
                              html`
                                ...
                                >
                                  ...
                                  >
                                    <img id="${() =>
                                      `most-skipped-tracks-image-${index}`}" class="w-full h-full" />
                                    <div
                                      id="${() =>
                                        `most-skipped-tracks-visualizer-${index}`}"
                                      class="absolute top-0 left-0 w-full h-full grid place-content-center text-white bg-black/30 hidden"
                                    >
                                    </div>
                                  </div>
                                  ...
                                  >
                                    <img id="${() =>
                                      `most-skipped-tracks-background-image-${index}`}" class="w-full h-full" />
                                  </div>
                                </li>
                              `.key(track.id)
                              // .key((Math.random() * 100000).toString(16)) // assign a random key to force re-rendering of the list (and thus the indices)
                ...
                ${() =>
                  !state.extraFeatures.mostSkippedTracks ? null : html`
                    ...
                    >
                      ${() =>
                        state.rewindReport.tracks?.[`mostSkipped`]?.slice(5, 20)
                          .map((track, index) =>
                            html`
                              <li class="relative overflow-hidden w-1/2 mx-auto pl-3">
                                <div class="flex flex-col gap-1 w-full">
              ...
              buildFeature(
                ...
                html`
                  <div class="text-center text-white">
                    ...
                    <div class="flex mt-10 flex-col">
                      <img id="most-successive-streams-track-image" class="${() =>
                        `w-[30vh] h-[30vh] mx-auto rounded-md drop-shadow-[0_35px_35px_rgba(255,255,255,0.25)] ${
                        ...
                        <div class="-rotate-6 -ml-10 mt-10 text-4xl font-semibold">
                          <div class="">${() =>
                            state.extraFeatures.mostSuccessivePlays
                              ? state.settings.useAlbumArtists
                                ? state.rewindReport.generalStats
                                  .mostSuccessivePlays?.albumArtist?.name
                                : state.rewindReport.generalStats
                                  .mostSuccessivePlays?.artists
                                  ?.reduce(
                                    (acc, cur, index) =>
                                      index > 0
                                        ? `${acc} & ${cur.name}`
                      ...
                      <div>
                        <span class="font-semibold">${() =>
                          state.extraFeatures.mostSuccessivePlays
                            ? showAsNumber(
                              state.rewindReport.generalStats
                                .mostSuccessivePlays?.playCount,
                            )
                            : `???`}</span> successive streams.
                      </div>
                      <div>
                        Adding up to <span class="font-semibold">${() =>
                          state.extraFeatures.mostSuccessivePlays
                            ? showAsNumber(
                              state.rewindReport.generalStats
                                .mostSuccessivePlays?.totalDuration.toFixed(1),
                            )
                            : `???`}</span> minutes.
                      </div>
                    </div>
                  ...
                  >
                    <img
                      id="most-successive-streams-track-background-image"
                      class="w-full h-full"
                    />
                  </div>
                  ${() =>
                ...
                html`
                  <div class="h-full p-4 flex flex-col justify-around">
                    ...
                    >
                      ...
                      >${() => state.rewindReport?.year}</span></span>
                    </h2>
                    ...
                    >
                      ...
                      >
                        ...
                        <div class="text-balance">
                          ...
                          >${() =>
                            state.rewindReport.tracks
                              ?.[state.settings.rankingMetric]?.[0]
                              ?.name}</span>
                          <span class="">by ${() =>
                            state.settings.useAlbumArtists
                              ? state.rewindReport.tracks
                                ?.[state.settings.rankingMetric]?.[0]
                                ?.albumBaseInfo?.albumArtistBaseInfo?.name
                              : state.rewindReport.tracks
                                ?.[state.settings.rankingMetric]?.[0]
                                ?.artistsBaseInfo?.reduce(
                                  (acc, cur, index) =>
                                    index > 0
                                      ? `${acc} & ${cur.name}`
                                      : cur.name,
                          ...
                          ? html`
                            ...
                            >
                              ...
                              <div class="text-xl text-balance">
                                Listened on <span
                                  class="font-semibold text-[#00A4DC] text-3xl font-quicksand"
                                >${() =>
                                  showAsNumber(
                                    state.rewindReport.generalStats
                                      .totalMusicDays,
                      ...
                      >
                        ...
                        <div class="text-balance">
                          ...
                          >${() =>
                            showAsNumber(
                              (state.rewindReport?.libraryStats?.totalRuntime /
                                60.0 / 60.0 / 24.0).toFixed(1),
                            )}</span> days
                        </div>
                        <div class="text-balance">
                          ...
                          >${() =>
                            showAsNumber(
                              state.rewindReport?.libraryStats?.trackLength
                                ?.mean.toFixed(0),
                            )}</span> seconds
                        </div>
                      </div>
                                  ...
                                  ? html`
                                    <div class="text-center mb-2">
                                      ...
                                      >${() =>
                                        ...
                                        )}</span> more streams than last year
                                    </div>
                                  `
                                  : null}
                ...
                html`
                  <div class="p-4">
                    ...
                    <div class="mt-4 w-full flex flex-col items-center">
                      <img class="h-16" src="${() =>
                        state.settings.darkMode
                          ? "/media/jellyfin-banner-dark.svg"
                          : "/media/jellyfin-banner-light.svg"}" alt="Jellyfin Rewind Logo">
                      <h3
                    ...
                    <button
                      ...
                      @click="${stopPropagation(() => {
                        if (!state.rewindReportData.rawData) {
                          ...
                          window.downloadRewindReportData(
                            state.rewindReportData,
                          );
                          ...
                          showOverlay({
                            ...
                            content: html`
                              ...
                              >
                                ...
                                <p class="">
                                  Without this data, you might be missing out on some insights next year, as
                                  well as improved quality of the statistics.
                                </p>
                                <p class="">
                                  If possible, please save the Rewind report somewhere safe until next year,
                                  just in case. It's only a few (hundred) MBs in size.
                                </p>
                              </div>
            ...
            function showIncompleteReportOverlay(onClose = () => {}) {
              ...
              showOverlay({
                ...
                content: html`
                  <div
                    class="flex flex-col gap-4 text-lg font-medium leading-6 text-gray-500 mt-10 w-full mx-auto text-balance text-center"
                  >
                    <p class="">
                      The Rewind Report you're about to download is incomplete and missing some
                      data.
                    </p>
                    <p class="">
                      Please re-generate and download the report without reloading the page
                      in-between.
                    </p>
                  </div>
                  ...
                  >
                    ...
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="w-7 h-7 stroke-[2.5] icon icon-tabler icon-tabler-refresh"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
            ...
            function showOverlayDataAccuracy() {
                  ...
                  <div class="flex flex-col items-start gap-2">
                    <p>
                      ...
                      playtime are only an approximation. It also means that it is <span
                        class="font-semibold"
                      >not possible to limit the data to ${() =>
                        state.rewindReport?.year} without plugins!</span>
                    </p>
                    <p>
                      However, if you have the <span class="font-semibold"
                      >"Playback Reporting"</span> plugin installed, <span class="font-semibold"
                      >significantly more information can be collected</span>, such as the date
                      and durations of each playback. This results in better stats, although it
                      isn't perfect either. Playback reporting depends on applications properly
                      reporting the current playback states, and currently most music players that
                      are compatible with Jellyfin seem to struggle with this in one way or
                      another. <span class="font-semibold"
                      >Offline playback is not recorded at all</span>.
                    </p>
                    <p>
                      Still, the best solution is to install the Playback Reporting plugin into
                      your Jellyfin server if you haven't done so already. It won't take longer
                      than 2 minutes, so why not do it right now? Your Jellyfin Rewind isn't going
                      anywhere!
                    </p>
                    ${() =>
                      state.auth.config?.user?.isAdmin
                        ? html`
                          <a
                            class="px-3 py-2 my-1 mx-auto rounded-md text-white font-semibold bg-[#00A4DC]"
                            href="${() =>
                              `${state.auth.config.baseUrl}/web/index.html#/dashboard/plugins/5c53438191a343cb907a35aa02eb9d2c?name=Playback%20Reporting`}"
                            target="_blank"
                          >Open Plugins Page!</a>
                        `
                        : html`
                          ...
                          <p class="font-medium mb-6">
                            ...
                            admin account, you can <a
                              class="text-[#00A4DC]"
                              href="${() =>
                                `${state.auth.config.baseUrl}/web/index.html#/dashboard/plugins/5c53438191a343cb907a35aa02eb9d2c?name=Playback%20Reporting`}"
                              target="_blank"
                            >click here to open the plugins page</a>. If nothing happens or Jellyfin just
                            keeps loading, that means the logged-in account is not an administrator.
                          </p>
                        `}
                    <p>
                      ...
                      state.auth.config?.user?.isAdmin
                        ? html`
                          <a
                            class="px-3 py-2 my-1 mx-auto rounded-md text-white font-semibold bg-[#00A4DC]"
                            href="${() =>
                              `${state.auth.config.baseUrl}/web/index.html#/configurationpage?name=playback_report_settings`}"
                            target="_blank"
                          >Open Settings</a>
                        `
                        : html`
                          ...
                          <p class="font-medium mb-6">
                            You're not logged in with an administrator account, but you need to be an
                            admin in order to install plugins. If you are logged into Jellyfin with an
                            admin account and already have the Playback Reporting plugin installed, you
                            can <a class="text-[#00A4DC]" href="${() =>
                              `${state.auth.config.baseUrl}/web/index.html#!/configurationpage?name=playback_report_settings`}" target="_blank"
                            >click here to open the plugin settings</a>. If nothing happens or Jellyfin
                            just keeps loading, that means the logged-in account is not an administrator.
                          </p>
                        `}
                    <p>
                    ...
                    <p>
                      So, please treat all of this information with a grain of salt. You can take
                      a look at the settings in order to choose which data will be used, but any
                      information that needs to be interpolated will have a negative influence on
                      the quality of these stats.
                    </p>
                    <p>
                      I will try to offer a way to import this year's Rewind data into next year's
                      Jellyfin Rewind, so that more information can be used and the used data can
                      be properly limited to the current year only. Because of this, please <span
                        class="font-semibold"
                      >make sure to download a copy of your Rewind data at the end and store it
                        until next year!</span>
                      </p>
                    </div>
                  `,
                });
                  ...
                  content: html`
                    <div class="flex flex-col items-start gap-2">
                      ...
                      <p class="mt-6">
                        ...
                        year, you can try it out in ${() =>
                          state.rewindReport?.year}!
                      </p>
                    </div>
                  `,
                });
              ...
              const settings = html`
                ${() =>
                  !state.extraFeatures.fullReport
                    ? html`
                      <p class="text-sm text-orange-500 dark:text-orange-400">
                        Jellyfin Rewind is using a 'light' version of the Rewind report, therefore
                        some settings are not available.
                      </p>
                      <p class="text-sm mt-1 mb-3 text-gray-500 dark:text-gray-400">
                        To access all settings, please <button
                          @click="${() => {
                            closeFeatures(); // close settings
                            closeFeatures(); // close features
                            closeFeatures(); // for good measure, in case an overlay is open
                            window.generateRewindReport().then(
                              (rewindReport) => {
                          ...
                          class="text-[#00A4DC]"
                        >
                          re-generate the Rewind report
                        </button> and make sure you have enough free storage left on your device!
                      </p>
                    `
                    : html`
                      <br>
                  ...
                  <li class="flex flex-row justify-between">
                    ${() =>
                      buildOptionChooser({
                        title: `Sound`,
                        description:
                          `Should fitting music be played while you view the report?`,
                        settingsKey: `sound`,
                        options: [
                          {
                            name: `On`,
                  ...
                  <li class="flex flex-row justify-between">
                    ${() =>
                      buildOptionChooser({
                        ...
                        description:
                          `Choose the main data source for the report`,
                        settingsKey: `dataSource`,
                        options: [
                          {
                            name: `Playback Reporting`,
                            description:
                              `Use the Playback Reporting Plugin (most accurate)`,
                            value: `playbackReport`,
                            disabled:
                              state.rewindReport.playbackReportDataMissing,
                          },
                          {
                            name: `Combined`,
                            description:
                              `Use the average of Jellyfin's built-in play count tracking and the Playback Reporting Plugin`,
                            value: `average`,
                            disabled:
                              state.rewindReport.playbackReportDataMissing,
                          },
                          {
                            name: `Jellyfin`,
                            description:
                              `Use Jellyfin's built-in play count tracking (least accurate)`,
                            value: `jellyfin`,
                          },
                        ],
                      })}
                  ...
                  <li class="flex flex-row justify-between">
                    ${() =>
                      buildOptionChooser({
                        title: `Rank by...`,
                        description:
                          `Choose which metric should be used for ranking the top tracks, artists, albums, genres, etc.`,
                        ...
                        options: [
                          ...
                          },
                          {
                            name: `Play Count`,
                            description:
                              `Only use the number of times an item was played, disregarding the duration of each play`,
                            value: `playCount`,
                          },
                        ],
                      })}
                  ...
                  <li class="flex flex-row justify-between">
                    ${() =>
                      buildOptionChooser({
                        title: `Artist Type`,
                        description:
                          `Toggle between (track) artists and album artists`,
              ...
              function forgottenFavoritesPageContent() {
                return html`
                  <div class="text-center">
                    ...
                    <h3 class="px-6">
                      Here are ${() =>
                        state.rewindReport.tracks?.forgottenFavoriteTracks
                          .length} tracks you loved earlier this year, but it's been a
                      while since you last played them.
                    </h3>
                          ...
                          .slice(0, 5).map((track, index) =>
                            html`
                              ...
                              >
                                ...
                                >
                                  <img id="${() =>
                                    `forgotten-tracks-image-${index}`}" class="w-full h-full" />
                                  <div
                                    id="${() =>
                                      `forgotten-tracks-visualizer-${index}`}"
                                    class="absolute top-0 left-0 w-full h-full grid place-content-center text-white bg-black/30 hidden"
                                  >
                                  </div>
                                </div>
                                ...
                                >
                                  <img id="${() =>
                                    `forgotten-tracks-background-image-${index}`}" class="w-full h-full" />
                                </div>
                              </li>
                            `.key(track.id)
                          )}
                  ...
                  >
                    ${() =>
                      state.rewindReport.tracks?.forgottenFavoriteTracks?.slice(
                        5,
                        10,
                      ).map((track, index) =>
                        html`
                            ...
                            <div class="flex flex-col gap-1 w-full">
                              <div class="flex flex-col gap-0.25 items-start">
                                ...
                                <div class="ml-6 max-h-[2rem] text-xs">
                                  ...
                                  >${() =>
                                    state.settings.useAlbumArtists
                                      ? track.albumBaseInfo.albumArtistBaseInfo
                                        .name
                                      : track.artistsBaseInfo
                                        .reduce((acc, cur, index) =>
                                          index > 0
              ...
              function closeOverlay(overlayId) {
                if (!overlayId) {
                  ...
                  let index = state.overlays.findIndex((x) =>
                    x.overlayId === overlayId
                  );
                  if (index > -1) {
              ...
              function buildOverlay({ title, content, overlayId, onClose }) {
                return html`
                  <div
                    style="${() => `z-index: ${200 + state.overlays.length}`}"
                    class="absolute top-0 left-0 w-full h-full px-6 py-16 md:py-32 lg:py-48 xl:py-64"
                  >
                    <div
                      @click="${() => onClose()}"
                    ...
                    <div
                      class="w-full h-full bg-white/80 dark:bg-black/90 dark:text-white pb-20 backdrop-blur dark:backdrop-blur-sm rounded-xl"
                    >
                      <div
              ...
              function buildOptionChooser(
                { title, description, settingsKey, options },
              ) {
                let selectedValue = state.settings[settingsKey];
                let selectedOptionIndex = options.findIndex((option, index) =>
                ...
                return html`
                  <div class="w-full flex flex-col gap-2 items-center">
                    ...
                    <div
                      class="w-full flex flex-row justify-around overflow-hidden border-4 border-gray-200 dark:border-gray-900 items-center text-sm rounded-full bg-gray-200 dark:bg-gray-900"
                    >
                      ${() =>
                        options.map((option, index) => {
                          const optionDisabled =
                            state.disabledSettings.includes(settingsKey) ||
                            option.disabled;

                          return html`
                            <button
                              class="w-full h-full rounded-md ${selectedOptionIndex ===
                                  index
                              ...
                              @click="${(e) =>
                                !optionDisabled
                                  ? updateSetting(settingsKey, option.value)
                                  : e.preventDefault()}"
                            >
                    ...
                    <p class="w-full text-center text-xs px-4 text-gray-600 dark:text-gray-300">
              ...
              watch(() => state.settings.rankingMetric, () => {
                ...
                setTimeout(() => {
                  Object.values(state.featureSideEffects).forEach(
                    (feature, index) => {
              ...
              export function init(rewindReportData, jellyHelper, auth) {
                ...
                console.log(
                  `state.rewindReport.type:`,
                  state.rewindReport.type,
                );
                console.log(
                  `state.rewindReport.type !== 'full':`,
                  state.rewindReport.type !== "full",
                );
                console.log(`state.jellyHelper:`, state.jellyHelper);
                console.log(`state.auth:`, state.auth);

                state.rewindReportDownloaded =
                  JSON.parse(localStorage.getItem(`rewindReportDownloaded`)) ||
                  false;

                if (state.rewindReport.type !== `full`) {
                  state.extraFeatures.fullReport = false;
                  state.settings.useAlbumArtists = true;
                  !state.disabledSettings.includes(`useAlbumArtists`)
                    ? state.disabledSettings.push(`useAlbumArtists`)
                    : null;
                } else {
                  state.extraFeatures.fullReport = true;
                  state.disabledSettings = state.disabledSettings.filter((
                    setting,
                  ) => setting !== `useAlbumArtists`);
                }
                ...
                state.extraFeatures.totalPlaytimeGraph =
                  state.rewindReport.playbackReportAvailable &&
                  !state.rewindReport.playbackReportDataMissing;
                ...
                if (
                  !state.rewindReport.featureDelta?.listeningActivityDifference
                ) {
                ...
                let content = html`
                  ${() => {
                    return state.featuresOpen
                      ? html`
                        <div
                          class="fixed top-0 left-0 w-[100vw] h-[100vh] bg-white flex flex-row justify-center"
                        >
                          <div class="absolute w-full md:max-w-3xl h-full">
                            <div
                              class="absolute top-0 left-0 z-[5] w-full h-10 flex flex-row justify-between bg-gray-700/30"
                            >
                              <ul
                                class="px-2 py-4 z-[100] w-full h-full flex flex-row gap-1.5 justify-between"
                              >
                                ${() => {
                                  return state.features.map(
                                    (feature, index) => {
                                      return html`
                                        <li class="${() =>
                                          `relative block w-full rounded-full h-full text-white/0 ${
                              ...
                              <button class="px-1 z-[150]" @click="${() =>
                                ...
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  class="w-7 h-7 text-white icon icon-tabler icon-tabler-settings"
                                  width="24"
                                  height="24"
                                  viewBox="0 0 24 24"
                                  stroke-width="2"
                              ...
                              <button class="px-1 z-[150]" @click="${() =>
                                closeFeatures()}" type="button">
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  class="w-7 h-7 text-white icon icon-tabler icon-tabler-x"
                                  viewBox="0 0 24 24"
                                  stroke-width="2"
                                  stroke="currentColor"
                                  fill="none"
                            ...
                            ${() =>
                              Object.values(state.overlays).map((x) =>
                                x.overlay
                              )}
                          </div>
              ...
              watch(() => state.featuresOpen, (value) => {
                if (value) {
                  ...
                  setTimeout(() => {
                    Object.entries(state.featureSideEffects).forEach(
                      ([featureId, sideEffects]) => {
                        sideEffects.load?.();
                      },
                    );
                  }, 500);
              ...
              watch(() => state.featuresOpen && state.currentFeature, () => {
                if (state.featuresOpen) {
                  ...
                  state.featureSideEffects[(index - 1) % state.features.length]
                    ?.leave?.();
                }
              });
              ...
              function next() {
                ...
                if (state.currentFeature >= state.features.length - 1) {
                  ...
                  if (
                    state.featureSideEffects[state.currentFeature + 1]?.skip
                  ) {
              ...
              function buildFeature(featureName, content, classes) {
                ...
                return (index) =>
                  html`
                    <li @click="${(e) =>
                      handleFeatureClick(e)}" data-feature-name="${() =>
                      featureName}" class="${() =>
                      `${classes} cursor-pointer [-webkit-tap-highlight-color:_transparent] absolute top-0 left-0 w-full h-full overflow-auto pt-8 pb-4 transition-opacity duration-700 ${
                        state.currentFeature === index
                          ? `opacity-100`
              ...
              function handleFeatureClick(event) {
                ...
                let featureElement = event?.target?.closest?.(
                  `[data-feature-name]`,
                );
                console.log(`featureElement:`, featureElement);
                console.log(`event.clientX:`, event?.clientX);
                console.log(
                  `featureElement.offsetLeft:`,
                  featureElement?.offsetLeft,
                );
                console.log(
                  `featureElement.getBoundingClientRect().x:`,
                  featureElement?.getBoundingClientRect?.()?.x,
                );
              ...
              function showPlaytimeByMonthChart() {
                ...
                let monthData = Object.keys(
                  state.rewindReport.generalStats.totalPlaybackDurationByMonth,
                ).reduce((acc, month) => {
                  acc[month] = state.rewindReport.generalStats
                    .totalPlaybackDurationByMonth[month];
                  return acc;
                }, []);
                ...
                let data = {
                  ...
                  datasets: [{
                    ...
                    data: state.extraFeatures.totalPlaytimeGraph ? monthData : [
                      300,
                      600,
                      367,
                      763,
              ...
              function loadTopTrackMedia() {
                ...
                const topTrackBackgroundImage = document.querySelector(
                  `#top-track-background-image`,
                );
                console.log(`img:`, topTrackPrimaryImage);
                const topTrackByDuration = state.rewindReport.tracks
                  ?.[state.settings.rankingMetric]?.[0];
                console.log(`topTrackByDuration:`, topTrackByDuration);
                state.jellyHelper.loadImage(
                  [topTrackPrimaryImage, topTrackBackgroundImage],
                  topTrackByDuration.image,
                  `track`,
                  state.settings.darkMode,
                );
              }
              ...
              function loadTopTracksMedia() {
                ...
                const topTracks = state.rewindReport.tracks
                  ?.[state.settings.rankingMetric]?.slice(0, 5);
              ...
              function loadForgottenFavoritesMedia() {
                const forgottenFavoriteTracks = state.rewindReport.tracks
              ...
              function loadTopArtistMedia() {
                ...
                const topTrackByDuration = state.rewindReport.artists
                  ?.[state.settings.rankingMetric]?.[0];
                console.log(`topTrackByDuration:`, topTrackByDuration);
                state.jellyHelper.loadImage(
              ...
              function loadTopArtistsMedia() {
                const topArtists = state.rewindReport.artists
                  ?.[state.settings.rankingMetric]?.slice(0, 5);
              ...
              function loadTopAlbumMedia() {
                const topAlbumPrimaryImage = document.querySelector(
                  `#top-album-image`,
                );
                const topAlbumBackgroundImage = document.querySelector(
                  `#top-album-background-image`,
                );
                console.log(`img:`, topAlbumPrimaryImage);
                const topAlbumByDuration = state.rewindReport.albums
                  ?.[state.settings.rankingMetric]?.[0];
                console.log(`topAlbumByDuration:`, topAlbumByDuration);
                state.jellyHelper.loadImage(
                  [topAlbumPrimaryImage, topAlbumBackgroundImage],
                  topAlbumByDuration.image,
                  `album`,
                  state.settings.darkMode,
                );
              }
              ...
              function loadTopAlbumsMedia() {
                const topAlbums = state.rewindReport.albums
                  ?.[state.settings.rankingMetric]?.slice(0, 5);
              ...
              function loadLeastSkippedTracksMedia() {
                ...
                const topTracks = state.rewindReport.tracks?.[`leastSkipped`]
                  ?.slice(0, 5);
              ...
              function loadMostSkippedTracksMedia() {
                ...
                const topTracks = state.rewindReport.tracks?.[`mostSkipped`]
                  ?.slice(0, 5);
              ...
              function loadMostSuccessivePlaysTrackMedia() {
                const mostSuccessivePlaysTrackPrimaryImage = document
                  .querySelector(`#most-successive-streams-track-image`);
                const mostSuccessivePlaysTrackBackgroundImage = document
                  .querySelector(
                    `#most-successive-streams-track-background-image`,
                  );
                ...
                state.jellyHelper.loadImage(
                  [
                    ...
                    mostSuccessivePlaysTrackBackgroundImage,
                  ],
                  state.rewindReport.generalStats.mostSuccessivePlays?.image,
                  `track`,
                  state.settings.darkMode,
                );
              }
              ...
              function loadSummary() {
                ...
                console.log(`img:`, topTrackPrimaryImage);
                const topTrack = state.rewindReport.tracks
                  ?.[state.settings.rankingMetric]?.[0];
                ...
                console.log(`img:`, topArtistPrimaryImage);
                const topArtist = state.rewindReport.artists
                  ?.[state.settings.rankingMetric]?.[0];
                ...
                console.log(`img:`, mostSuccessivePlaysTrackPrimaryImage);
                const mostSuccessivePlaysTrack = state.rewindReport.generalStats
                  ?.mostSuccessivePlays?.track;
                console.log(
              ...
              function playTopTracks() {
                const topTracks = state.rewindReport.tracks
                  ?.[state.settings.rankingMetric]?.slice(1, 5); // first track excluded
              ...
              async function playTopArtist() {
                const topArtistByDuration = state.rewindReport.artists
                  ?.[state.settings.rankingMetric]?.[0];
                console.log(`topArtistByDuration:`, topArtistByDuration);

                let artistsTracks = await state.jellyHelper.loadTracksForGroup(
                  topArtistByDuration.id,
                  `artist`,
                );
                let randomTrackId = Math.floor(
                  Math.random() * artistsTracks.length,
                );
              ...
              async function playTopArtists() {
                const topArtists = state.rewindReport.artists
                  ?.[state.settings.rankingMetric]?.slice(1, 5); // first artist excluded
                const randomArtistId = Math.floor(
                  Math.random() * topArtists.length,
                );
              ...
              async function playTopAlbum() {
                ...
                let albumsTracks = await state.jellyHelper.loadTracksForGroup(
                  topAlbumByDuration.id,
                  `album`,
                );
                let randomTrackId = Math.floor(
                  Math.random() * albumsTracks.length,
                );
              ...
              async function playTopAlbums() {
                const topAlbums = state.rewindReport.albums
                  ?.[state.settings.rankingMetric]?.slice(1, 5); // first album excluded
                const randomAlbumId = Math.floor(
                  Math.random() * topAlbums.length,
                );
              ...
              async function playTopGenres() {
                const topGenres = state.rewindReport.genres
                  ?.[state.settings.rankingMetric]?.slice(0, 5); // first genre excluded
                const randomGenreId = Math.floor(
                  Math.random() * topGenres.length,
                );
                ...
                let genresTracks = await state.jellyHelper.loadTracksForGroup(
                  randomGenre.id,
                  `genre`,
                );
                let randomTrackId = Math.floor(
                  Math.random() * genresTracks.length,
                );
              ...
              function playLeastSkippedTracks() {
                const leastSkippedTracks = state.rewindReport.tracks
                  ?.[`leastSkipped`]?.slice(0, 5); // first track excluded
                const randomTrackId = Math.floor(
                  Math.random() * leastSkippedTracks.length,
                );
                const randomTrack = leastSkippedTracks[randomTrackId];
                showPlaying(
                  `#least-skipped-tracks-visualizer`,
                  randomTrackId,
                  5,
                );
              ...
              function playMostSkippedTracks() {
                const mostSkippedTracks = state.rewindReport.tracks
                ...
                showPlaying(
                  `#most-skipped-tracks-visualizer`,
                  randomTrackId,
                  5,
                );
              ...
              function showPlaying(itemQuery, itemId, idRange) {
                for (let i = 0; i < idRange; i++) {
                  const itemOverlay = document.querySelector(
                    `${itemQuery}-${i}`,
                  );
                  if (itemOverlay) {
                ...
                const playingItemOverlay = document.querySelector(
                  `${itemQuery}-${itemId}`,
                );
    ...
    `;
              ...
              async function fadeToNextTrack(trackInfo) {
                ...
                if (state.settings.sound) {
                  ...
                  const fadeStepsOut = Array(1 / fadePerStep).fill(1).map((
                    _,
                    i,
                  ) => Number((1 - i * fadePerStep).toFixed(2)));
                  const fadeStepsIn = Array(1 / fadePerStep).fill(1).map((
                    _,
                    i,
                  ) => Number((i * fadePerStep).toFixed(2)));
              ...
              function pausePlayback() {
                ...
                const fadeSteps = Array(1 / fadePerStep).fill(1).map((_, i) =>
                  Number((1 - i * fadePerStep).toFixed(2))
                );
              ...
              function showAsNumber(numberOrArray) {
                ...
                return numberOrArray?.toString()?.replace(
                  /\B(?=(\d{3})+(?!\d))/g,
                  ",",
                );
              }
