  ...
  const topTrackInfo = Object.values(itemInfo).map((item) => {
    try {
      ...
      const track = new Track({
        ...
        id: item.Id,
        artistsBaseInfo: item.ArtistItems.map((artist) => ({
          id: artist.Id,
        ...
        year: item.PremiereDate
          ...
          : null,
        duration: !isNaN(Math.round(item.RunTimeTicks / 10000000))
          ? Math.round(item.RunTimeTicks / 10000000)
          : 0,
        skips: {
          partial: playbackReportItem?.PartialSkips || 0,
          full: playbackReportItem?.FullSkips || 0,
          total: (playbackReportItem?.PartialSkips || 0) +
            (playbackReportItem?.FullSkips || 0),
        ...
        playCount: {
          ...
          average: Math.ceil(
            ((item.UserData?.PlayCount || 0) +
              Number(adjustedPlaybackReportPlayCount || 0)) / 2,
          ),
        },
      ...
      track.skips.score.jellyfin = (track.skips.total + 1) * 2 /
        track.playCount.jellyfin;
      track.skips.score.playbackReport = (track.skips.total + 1) * 2 /
        track.playCount.playbackReport;
      track.skips.score.average = (track.skips.total + 1) * 2 /
        track.playCount.average;
  ...
  const topAlbumInfo = topTrackInfo.reduce((acc, cur) => {
    ...
    if (!acc[albumId]) {
      acc[albumId] = new Album({
        ...
        tracks: [cur],
        year: currentAlbumInfo?.PremiereDate
          ? new Date(currentAlbumInfo.PremiereDate)
          : cur.year,
        image: new PrimaryImage({
          parentItemId: albumId,
          primaryTag: currentAlbumInfo?.ImageTags?.Primary,
          blurhash: currentAlbumInfo?.ImageBlurHashes?.Primary
            ?.[currentAlbumInfo?.ImageTags?.Primary],
        }),
        playCount: {
      ...
      cur.artistsBaseInfo.forEach((artistInfo) =>
        acc[albumId].artists.add(artistInfo)
      );
      acc[albumId].playCount.jellyfin += Number(cur.playCount?.jellyfin);
      acc[albumId].playCount.playbackReport += Number(
        cur.playCount?.playbackReport,
      );
      acc[albumId].playCount.average = Math.ceil(
        ((acc[albumId]?.playCount?.jellyfin || 0) +
          (acc[albumId]?.playCount?.playbackReport || 0)) / 2,
      );
      acc[albumId].plays.concat(cur.plays);
      acc[albumId].lastPlayed = (acc[albumId]?.lastPlayed || 0) > cur.lastPlayed
        ? (acc[albumId]?.lastPlayed || 0)
        : cur.lastPlayed;
      acc[albumId].totalPlayDuration.jellyfin += Number(
        cur.totalPlayDuration.jellyfin,
      );
      acc[albumId].totalPlayDuration.playbackReport += Number(
        cur.totalPlayDuration.playbackReport,
      );
      acc[albumId].totalPlayDuration.average = Math.ceil(
        ((acc[albumId]?.totalPlayDuration?.jellyfin || 0) +
          (acc[albumId]?.totalPlayDuration?.playbackReport || 0)) / 2,
      );
    }
    return acc;
  }, {});
  ...
  const topArtistInfo = topTrackInfo.reduce((acc, cur) => {
    cur.artistsBaseInfo.forEach((artist) => {
      ...
      if (!acc[artistId]) {
        acc[artistId] = new Artist({
          ...
          images: {
            primary: new PrimaryImage({
              ...
              blurhash: currentArtistInfo?.ImageBlurHashes?.Primary
                ?.[currentArtistInfo.ImageTags.Primary],
            }),
            backdrop: new BackdropImage({
              id: 0,
              parentItemId: artistId,
              backgroundTag: currentArtistInfo?.BackdropImageTags?.[0],
              blurhash: currentArtistInfo?.ImageBlurHashes?.Backdrop
                ?.[currentArtistInfo.BackdropImageTags?.[0]],
            }),
          },
          playCount: {
            jellyfin: cur.playCount?.jellyfin,
          ...
          uniquePlayedTracks: {
            jellyfin: new Set(
              cur.playCount?.jellyfin > 0
                ? [{ id: cur.id, name: cur.name }]
                : [],
            ),
            playbackReport: new Set(
              cur.playCount?.playbackReport > 0
                ? [{ id: cur.id, name: cur.name }]
                : [],
            ),
            average: new Set(
              cur.playCount?.average > 0
                ? [{ id: cur.id, name: cur.name }]
                : [],
            ),
          },
        ...
        acc[artistId].playCount.playbackReport += Number(
          cur.playCount?.playbackReport,
        );
        acc[artistId].playCount.average = Math.ceil(
          ((acc[artistId]?.playCount?.jellyfin || 0) +
            (acc[artistId]?.playCount?.playbackReport || 0)) / 2,
        );
        acc[artistId].uniqueTracks.add({ id: cur.id, name: cur.name });
        if (cur.playCount?.jellyfin > 0) {
          acc[artistId].uniquePlayedTracks.jellyfin.add({
            id: cur.id,
            name: cur.name,
          });
        }
        if (cur.playCount?.playbackReport > 0) {
          acc[artistId].uniquePlayedTracks.playbackReport.add({
            id: cur.id,
            name: cur.name,
          });
        }
        if (cur.playCount?.average > 0) {
          acc[artistId].uniquePlayedTracks.average.add({
            id: cur.id,
            name: cur.name,
          });
        }
        acc[artistId].plays.concat(cur.plays);
        acc[artistId].lastPlayed =
          (acc[artistId]?.lastPlayed || 0) > cur.lastPlayed
            ? (acc[artistId]?.lastPlayed || 0)
            : cur.lastPlayed;
        acc[artistId].totalPlayDuration.jellyfin += Number(
          cur.totalPlayDuration.jellyfin,
        );
        acc[artistId].totalPlayDuration.playbackReport += Number(
          cur.totalPlayDuration.playbackReport,
        );
        acc[artistId].totalPlayDuration.average = Math.ceil(
          ((acc[artistId]?.totalPlayDuration?.jellyfin || 0) +
            (acc[artistId]?.totalPlayDuration?.playbackReport || 0)) / 2,
        );
      }
    });
    return acc;
  ...
  const topGenreInfo = topTrackInfo.reduce((acc, cur) => {
    cur.genreBaseInfo.forEach((genre) => {
      ...
      if (!acc[genreId]) {
        ...
        acc[genreId].playCount.playbackReport += Number(
          cur.playCount?.playbackReport,
        );
        acc[genreId].playCount.average = Math.ceil(
          ((acc[genreId]?.playCount?.jellyfin || 0) +
            (acc[genreId]?.playCount?.playbackReport || 0)) / 2,
        );
        acc[genreId].uniqueTracks.add({ id: cur.id, name: cur.name });
        if (cur.playCount?.jellyfin > 0) {
          acc[genreId].uniquePlayedTracks.jellyfin.add({
            id: cur.id,
            name: cur.name,
          });
        }
        if (cur.playCount?.playbackReport > 0) {
          acc[genreId].uniquePlayedTracks.playbackReport.add({
            id: cur.id,
            name: cur.name,
          });
        }
        if (cur.playCount?.average > 0) {
          acc[genreId].uniquePlayedTracks.average.add({
            id: cur.id,
            name: cur.name,
          });
        }
        acc[genreId].plays.concat(cur.plays);
        acc[genreId].lastPlayed =
          (acc[genreId]?.lastPlayed || 0) > cur.lastPlayed
            ? (acc[genreId]?.lastPlayed || 0)
            : cur.lastPlayed;
        acc[genreId].totalPlayDuration.jellyfin += Number(
          cur.totalPlayDuration.jellyfin,
        );
        acc[genreId].totalPlayDuration.playbackReport += Number(
          cur.totalPlayDuration.playbackReport,
        );
        acc[genreId].totalPlayDuration.average = Math.ceil(
          ((acc[genreId]?.totalPlayDuration?.jellyfin || 0) +
            (acc[genreId]?.totalPlayDuration?.playbackReport || 0)) / 2,
        );
      }
    });
    return acc;
...
export function generateTotalStats(topTrackInfo, enhancedPlaybackReport) {
  const totalStats = topTrackInfo.reduce((acc, cur) => {
    ...
    acc.totalPlayCount.playbackReport +=
      Number(cur.playCount?.playbackReport) || 0;
    acc.totalPlayCount.average = Math.ceil(
      (acc.totalPlayCount.jellyfin + acc.totalPlayCount.playbackReport) / 2,
    );
    acc.totalPlayDuration.jellyfin += Number(cur.totalPlayDuration.jellyfin);
    acc.totalPlayDuration.playbackReport += Number(
      cur.totalPlayDuration.playbackReport,
    );
    acc.totalPlayDuration.average = Math.ceil(
      (acc.totalPlayDuration.jellyfin + acc.totalPlayDuration.playbackReport) /
        2,
    );
    ...
    cur.artistsBaseInfo.forEach((artist) => acc.uniqueArtists.add(artist.id));
    ...
    cur.plays.forEach((play) => {
      ...
      acc.locations.devices[play.device] = acc.locations.devices[play.device]
        ? acc.locations.devices[play.device] + 1
        : 1;
      acc.locations.clients[play.client] = acc.locations.clients[play.client]
        ? acc.locations.clients[play.client] + 1
        : 1;
      ...
      if (!acc.minutesPerDay[play.date?.toLocaleDateString()]) {
        acc.minutesPerDay[play.date?.toLocaleDateString()] =
          Number(play.duration) / 60.0; // convert to minutes
      } else {
        acc.minutesPerDay[play.date?.toLocaleDateString()] +=
          Number(play.duration) / 60.0; // convert to minutes
      }
    ...
    if (
      cur.mostSuccessivePlays &&
      (!acc.mostSuccessivePlays ||
        cur.mostSuccessivePlays.playCount > acc.mostSuccessivePlays.playCount)
    ) {
      acc.mostSuccessivePlays = {
        track: cur,
        name: cur.name,
  ...
  enhancedPlaybackReport.notFound.forEach((item) => {
    ...
    totalStats.totalPlayCount.average = Math.ceil(
      (totalStats.totalPlayCount.jellyfin +
        totalStats.totalPlayCount.playbackReport) / 2,
    );
    totalStats.totalPlayDuration.playbackReport += Number(item.PlayDuration) /
      60;
    totalStats.totalPlayDuration.average = Math.ceil(
      (totalStats.totalPlayDuration.jellyfin +
        totalStats.totalPlayDuration.playbackReport) / 2,
    );
  });
  ...
  totalStats.minutesPerDay = {
    ...
    median: Object.values(totalStats.minutesPerDay).length % 2 === 0
      ? (Object.values(
        totalStats.minutesPerDay,
      )[Object.values(totalStats.minutesPerDay).length / 2] +
        Object.values(
          totalStats.minutesPerDay,
        )[Object.values(totalStats.minutesPerDay).length / 2 - 1]) / 2
      : Object.values(
        totalStats.minutesPerDay,
      )[Math.floor(Object.values(totalStats.minutesPerDay).length / 2)],
  };
  ...
  totalStats.libraryStats.trackLength.mean =
    totalStats.libraryStats.totalRuntime / totalStats.libraryStats.tracks.total;
  const sortedTrackLengths = Object.keys(
    totalStats.libraryStats.trackLength.lengths,
  ).sort((a, b) =>
    totalStats.libraryStats.trackLength.lengths[a] -
    totalStats.libraryStats.trackLength.lengths[b]
  );
  totalStats.libraryStats.trackLength.median =
    sortedTrackLengths.length % 2 === 0
      ? (totalStats.libraryStats.trackLength
        .lengths[sortedTrackLengths[sortedTrackLengths.length / 2]] +
        totalStats.libraryStats.trackLength
          .lengths[sortedTrackLengths[sortedTrackLengths.length / 2 - 1]]) / 2
      : totalStats.libraryStats.trackLength
...
export function getforgottenFavoriteTracks(
  itemInfo,
  { dataSource = `average` },
) {
  const minimumLastPlayAge = 120; // in days
  const numberOfTracksToReturn = 20;
  const trackList = Array.isArray(itemInfo)
    ? [...itemInfo]
    : Object.values(itemInfo);
  ...
  for (let i = 0; i < playedSongs.length; i++) {
    if (
      (today - playedSongs[i].lastPlay) / millisecondsPerDay >
        minimumLastPlayAge
    ) {
...
export function getTopItems(
  itemInfo,
  { by = `duration`, lowToHigh = false, limit = 25, dataSource = `average` },
) {
  ...
  topItems.sort((a, b) => {
    if (by === `duration`) {
      ...
      let result = b.playCount[dataSource] <= 2
        ? (a.playCount[dataSource] <= 2 ? 0 : -1)
        : (a.playCount[dataSource] <= 2
          ? 1
          : lowToHigh
          ? (a.skips.score[dataSource] - b.skips.score[dataSource])
          : (b.skips.score[dataSource] - a.skips.score[dataSource]));
      return result;
...
export function generateTotalPlaybackDurationByMonth(indexedPlaybackReport) {
  const totalPlaybackDurationByMonth = Object.values(indexedPlaybackReport)
    .reduce((acc, cur) => {
      cur.Plays.forEach((play) => {
  ...
  Object.keys(totalPlaybackDurationByMonth).forEach((month) => {
    totalPlaybackDurationByMonth[month] = Math.ceil(
      totalPlaybackDurationByMonth[month] / 60,
    ); // convert to minutes
  });
