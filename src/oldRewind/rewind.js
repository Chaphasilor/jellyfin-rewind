...
async function loadPlaybackReport(year) {
  const response = await fetch(
    `${auth.config.baseUrl}/user_usage_stats/submit_custom_query?stamp=${Date.now()}`,
    {
      method: "POST",
      headers: {
...
function generateJSONFromPlaybackReport(playbackReportInfo) {
  if (
    !playbackReportInfo || !playbackReportInfo.results ||
    !playbackReportInfo.colums
  ) {
    return {
      items: [],
    };
...
function adjustPlaybackReportJSON(playbackReportJSON, indexedItemInfo) {
  ...
  for (const index in playbackReportJSON.items) {
    ...
    if (
      playbackReportJSON.items[index].PlayDuration > jellyfinItemDuration + 1
    ) {
      console.debug(
        `Wrong duration for ${item.ItemId} (${item.ItemName}), adjusting from ${
          playbackReportJSON.items[index].PlayDuration
        } to ${jellyfinItemDuration}`,
      );
...
function indexPlaybackReport(playbackReportJSON) {
  ...
  for (const item of playbackReportJSON.items) {
    ...
    const playInfo = {
      ...
      duration: !isNaN(Number(item.PlayDuration))
        ? Number(item.PlayDuration)
        : 0,
    ...
    if (!items[item.ItemId]) {
      items[item.ItemId] = {
        ...item,
        TotalDuration: !isNaN(Number(item.PlayDuration))
          ? Number(item.PlayDuration)
          : 0,
        TotalPlayCount: 1,
        Plays: [playInfo],
      ...
      items[item.ItemId].TotalDuration += !isNaN(Number(item.PlayDuration))
        ? Number(item.PlayDuration)
        : 0;
    ...
    if (
      ...
      currentSuccessivePlays.totalDuration = !isNaN(Number(item.PlayDuration))
        ? Number(item.PlayDuration)
        : 0;
    } else {
      currentSuccessivePlays.count += 1;
...
export async function loadItemInfoBatched(items) {
  ...
  for (
    let batchIndex = 0;
    batchIndex < Math.ceil(items.length / batchSize);
    batchIndex++
  ) {
    console.info(`Fetching item batch info`);
    response = await loadItemInfo(
      items.slice(batchSize * batchIndex, batchSize * (batchIndex + 1)),
      auth,
    );
    if (!combinedResponse) {
...
async function loadArtistInfo(items) {
  ...
  response = await fetch(
    `${auth.config.baseUrl}/Artists/AlbumArtists?${queryParams}`,
    {
      method: "GET",
      headers: {
        ...auth.config.defaultHeaders,
...
function adjustItemInfo(itemInfo, oldReport) {
  ...
  for (const itemId of Object.keys(itemInfo)) {
    ...
    if (oldItemInfo) {
      ...
      if (
        itemInfo[itemId].UserData.PlayCount >= oldItemInfo.UserData.PlayCount
      ) {
        adjustedPlayCount = itemInfo[itemId].UserData.PlayCount -
          oldItemInfo.UserData.PlayCount;
      } else {
        adjustedPlayCount = itemInfo[itemId].UserData.PlayCount;
      }
...
async function generateRewindReport({
  ...
  try {
    ...
    const playbackReportJSON = generateJSONFromPlaybackReport(
      playbackReportInfo,
    );
    ...
    const enhancedPlaybackReportJSON = adjustPlaybackReportJSON(
      playbackReportJSON,
      allItemInfoAdjusted,
    );
    progressCallback(0.5);
    const indexedPlaybackReport = indexPlaybackReport(
      enhancedPlaybackReportJSON,
    );
    console.log(`indexedPlaybackReport:`, indexedPlaybackReport);
    progressCallback(0.6);

    console.log(
      `Object.keys(allItemInfoAdjusted).length:`,
      Object.keys(allItemInfoAdjusted).length,
    );
    const allTopTrackInfo = aggregate.generateTopTrackInfo(
      allItemInfoAdjusted,
    ...
    const totalStats = aggregate.generateTotalStats(
      allTopTrackInfo,
      enhancedPlaybackReportJSON,
    );
    ...
    jellyfinRewindReport.generalStats[`totalPlaybackDurationByMonth`] =
      aggregate.generateTotalPlaybackDurationByMonth(indexedPlaybackReport);
    if (
      Object.values(
        jellyfinRewindReport.generalStats[`totalPlaybackDurationByMonth`],
      ).filter((month) => month > 0).length < 12
    ) {
    ...
    const dataSource = playbackReportAvailable
      ? (playbackReportComplete ? `playbackReport` : `average`)
      : `jellyfin`;
    ...
    jellyfinRewindReport.generalStats[`totalPlaybackDurationMinutes`] = {
      playbackReport: Number(
        totalStats.totalPlayDuration[`playbackReport`].toFixed(1),
      ),
    ...
    jellyfinRewindReport.generalStats[`totalPlaybackDurationHours`] = {
      playbackReport: Number(
        (totalStats.totalPlayDuration[`playbackReport`] / 60).toFixed(1),
      ),
      average: Number(
        (totalStats.totalPlayDuration[`average`] / 60).toFixed(1),
      ),
      jellyfin: Number(
        (totalStats.totalPlayDuration[`jellyfin`] / 60).toFixed(1),
      ),
    };
    jellyfinRewindReport.generalStats[`uniqueTracksPlayed`] =
      totalStats.uniqueTracks;
    jellyfinRewindReport.generalStats[`uniqueAlbumsPlayed`] =
      totalStats.uniqueAlbums;
    jellyfinRewindReport.generalStats[`uniqueArtistsPlayed`] =
      totalStats.uniqueArtists;

    jellyfinRewindReport.generalStats[`playbackMethods`] =
      totalStats.playbackMethods;
    jellyfinRewindReport.generalStats[`locations`] = totalStats.locations;

    jellyfinRewindReport.generalStats[`mostSuccessivePlays`] =
      totalStats.mostSuccessivePlays;
    jellyfinRewindReport.generalStats[`totalMusicDays`] =
      totalStats.totalMusicDays;
    jellyfinRewindReport.generalStats[`minutesPerDay`] =
      totalStats.minutesPerDay;

    jellyfinRewindReport.libraryStats = totalStats.libraryStats;

    const topTracksByDuration = aggregate.getTopItems(allTopTrackInfo, {
      by: `duration`,
      limit: 20,
      dataSource: dataSource,
    });
    const topTracksByPlayCount = aggregate.getTopItems(allTopTrackInfo, {
      by: `playCount`,
      limit: 20,
      dataSource: dataSource,
    });
    const topTracksByLeastSkipped = aggregate.getTopItems(allTopTrackInfo, {
      by: `skips`,
      lowToHigh: true,
      limit: 20,
      dataSource: dataSource,
    });
    const topTracksByMostSkipped = aggregate.getTopItems(allTopTrackInfo, {
      by: `skips.full`,
      limit: 20,
      dataSource: dataSource,
    });
    // const topTracksByLastPlayed = aggregate.getTopItems(allTopTrackInfo, { by: `lastPlayed`, limit: 20, dataSource: dataSource })
    const forgottenFavoriteTracks = aggregate.getforgottenFavoriteTracks(
      allTopTrackInfo,
      { dataSource: dataSource },
    );
    ...
    jellyfinRewindReport.tracks[`forgottenFavoriteTracks`] =
    ...
    const topAlbumInfo = aggregate.generateAlbumInfo(
      allTopTrackInfo,
      albumInfo,
    );
    const topAlbumsByDuration = aggregate.getTopItems(topAlbumInfo, {
      by: `duration`,
      limit: 20,
      dataSource: dataSource,
    });
    const topAlbumsByPlayCount = aggregate.getTopItems(topAlbumInfo, {
      by: `playCount`,
      limit: 20,
      dataSource: dataSource,
    });
    ...
    const topArtistInfo = aggregate.generateArtistInfo(
      allTopTrackInfo,
      artistInfo,
    );
    const topArtistsByDuration = aggregate.getTopItems(topArtistInfo, {
      by: `duration`,
      limit: 20,
      dataSource: dataSource,
    });
    const topArtistsByPlayCount = aggregate.getTopItems(topArtistInfo, {
      by: `playCount`,
      limit: 20,
      dataSource: dataSource,
    });
    ...
    const topGenresByDuration = aggregate.getTopItems(topGenreInfo, {
      by: `duration`,
      limit: 20,
      dataSource: dataSource,
    });
    const topGenresByPlayCount = aggregate.getTopItems(topGenreInfo, {
      by: `playCount`,
      limit: 20,
      dataSource: dataSource,
    });
    ...
    if (oldReport) {
      const featureDelta = await getFeatureDelta(oldReport, {
...
function saveRewindReport() {
  if (rewindReport !== null) {
    try {
      ...
      console.warn(
        `Couldn't save full report to localStorage (maybe quota exceeded?). Saving a subset only`,
      );
      const reduceToSubsets = (categoryEntry) =>
        Object.entries(categoryEntry).reduce((acc, [key, value]) => {
          console.log(`key:`, key);
...
function restoreRewindReport() {
  ...
  if (rewindReportJSON !== null) {
    ...
    console.warn(
      `No full Rewind report to restore! Attempting to restore light report...`,
    );
  ...
  if (rewindReport.commit !== __COMMITHASH__) {
    console.warn(
      `Rewind report was generated with a different version of the app!`,
    );
  }
  if (rewindReport.year !== new Date().getFullYear()) {
    console.warn(
      `Rewind report was generated for a different year (${rewindReport.year})!`,
    );
  }
