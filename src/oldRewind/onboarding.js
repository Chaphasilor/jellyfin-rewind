...
import {
  ...
  loginViaAuthToken,
  loginViaPassword,
  restoreAndPrepareRewind,
} from "./setup";
import { getFeatureDelta, importRewindReport } from "./delta";
import {
  checkIfOfflinePlaybackImportAvailable,
  importOfflinePlayback,
  Play,
  uploadOfflinePlaybackBatched,
} from "./offline-import";
  ...
  html`
    <div class="md:max-w-3xl mx-auto">
      ${() => state.views[state.currentView]} ${() =>
        ...
        state.playbackReportingDialogOpen
          ? playbackReportingDialog
          : null} ${() =>
...
const viewStart = html`
  <div class="p-4">
    ...
    >
      ...
      <p class="">
        Before we can get started with your rewind, you'll have to log into your
        Jellyfin server.
      </p>
      <p class="">
        Ideally, your server is reachable over the internet and via secure HTTPS,
        but even if not, there are ways to enjoy your Rewind.
      </p>
...
const viewPlaceholder = html`
  <div class="p-4">
    ...
    >
      ...
      <p class="">
        If you're looking for this year's Jellyfin Rewind, you'll have to wait a
        little longer. <span class="text-[#00A4DC] font-semibold dark:text-white"
        >Jellyfin Rewind ${Number(import.meta.env.VITE_TARGET_YEAR) +
          1}</span> will launch on <span
          class="text-[#00A4DC] font-semibold dark:text-white"
        >December 31st, ${Number(import.meta.env.VITE_TARGET_YEAR) +
          1}</span> (if all goes well).
      </p>
      <p class="">
        In order to prepare for the launch, make sure your Playback Reporting
        plugin is installed and set up properly.
      </p>
      <button
        class="self-center text-white font-semibold px-6 py-2 rounded-2xl bg-orange-500 dark:text-white"
        @click="${() => state.playbackReportingDialogOpen = true}"
      >
        Click here<br>to configure it!
      </button>

      <p class="mt-12 font-normal text-base italic">
        If you want to review your stats from last year, you can now import your
        old report for viewing. The media itself probably won't load, but
    ...
    <div
      class="flex flex-col gap-2 leading-5 text-gray-500 dark:text-gray-400 w-full mx-auto text-balance text-center mt-12 font-normal text-base italic"
    >
      <p>
        If you really can't wait though, Jellyfin Rewind ${Number(
          import.meta.env.VITE_TARGET_YEAR,
        )} should still
        work just fine. It still has the same features as last year. Just be aware
        that there's more to come, and you will probably have an even better
        experience and more accurate stats if you wait just a bit longer!
      </p>
...
const connectionHelpDialog = html`
  <div
    class="fixed top-0 left-0 w-full h-full px-6 py-32 md:py-24 lg:py-28 xl:py-32"
  >
    <div
      @click="${() => state.connectionHelpDialogOpen = false}"
      ...
      <div class="w-full h-full overflow-x-auto p-4">
        <div class="flex flex-col items-start gap-2">
          <p>
            ...
            (HTTP<span class="font-semibold">S</span>), or requests from a
            non-private context (website not within your network) to a private
            context (Jellyfin server accessed over a local IP address within your
            network).
          </p>
          ${() =>
            window.location.protocol === `https:`
              ? html`
                <p>
                  Make sure that your Jellyfin server is accessible via <b>HTTPS</b>. If your
                  server is only available via regular HTTP, try going to <a
                    class="text-[#00A4DC]"
          ...
          <p>
            So make sure you're not using a local IP address (starts with <span
              class="font-mono"
            >192.168.</span>) or mDNS hostname (something like <span
              class="font-mono"
            >jellyfin.local</span>). If you use something like Tailscale as your
            VPN, you could use your server's Tailscale IP address.
          </p>
          <p>
            ...
            <p>
              If your server <span class="font-semibold">is NOT</span> accessible
              over the internet, you could self-host the Jellyfin Rewind website on
              your local network, for example on the same server that is running
              Jellyfin. For that, check out the <a
                class="text-[#00A4DC]"
  ...
  const playbackReportingDialog = html`
    ...
    >
      <div
        @click="${() => state.playbackReportingDialogOpen = false}"
        class="absolute top-0 left-0 w-full h-full bg-black/20"
      >
      </div>
      <div
        ...
        <div class="w-full h-full overflow-x-auto p-4">
          <div class="flex flex-col items-start gap-2">
            <p>
              ...
              >not possible to limit the data to a specific time frame, like
                ${import.meta.env.VITE_TARGET_YEAR} only!</span>
              </p>
              <p>
              ...
              <p>
                ...
                take longer than 2 minutes, so why not do it right now? (You'll have
                to be logged in as an admin user.)
              </p>
              ${() =>
                state.server.url !== ``
                  ? html`
              ...
              <p>
                I will try to again offer a way to import ${() =>
                  state.rewindReport?.year}'s Rewind data
                into Jellyfin Rewind ${() =>
                  state.rewindReport?.year +
                  1}, so that more information can be used
                and the used data can be properly limited to the current year only.
              </p>
            </div>
          </div>
        </div>
    ...
    const finampOfflineExportDialog = html`
      ...
      >
        <div
          @click="${() => state.finampOfflineExportDialogOpen = false}"
          class="absolute top-0 left-0 w-full h-full bg-black/20"
        >
        </div>
        <div
        ...
        >
          ...
          <div class="w-full h-full overflow-x-auto p-4">
            <div class="flex flex-col items-start gap-2">
              <p>
                If you're using Finamp's beta version, you can easily export your
                offline plays to a file, and then import that file here into Jellyfin
                Rewind. Just follow these steps:
              </p>
              <ol>
                <li>1. Open Finamp</li>
                ...
                <li>
                  5. Save the file, for example by sharing it to a file manager or
                  sending it to yourself via email
                </li>
              </ol>
              <p>
                Once you're ready, click the button below, and then click the "Import
                Offline Playback History" button and select the file you just exported
              </p>
              <button
                class="px-3 py-2 my-1 mx-auto rounded-md text-white font-semibold bg-[#00A4DC]"
                @click="${() => {
    ...
    async function connect(url) {
      ...
      if (url) {
        ...
        state.server.url =
          document.querySelector(`#onboarding-server-url`).value;
      }
      // remove trailing slash
      state.server.url = state.server.url.replace(/\/$/, ``);
      try {
        state.server.users = await connectToServer(
          state.auth,
          state.server.url,
        );
        ...
        state.error = html`
          <div
            class="flex flex-col items-start gap-1 text-base font-medium leading-6 text-red-500 dark:text-red-400 mt-10 w-5/6 mx-auto"
          >
            <p class="">There was an error while connecting to the server.</p>
            <p class="font-mono">${err.toString()}</p>
            <button
              class="self-center text-[#00A4DC] font-semibold px-3 py-1 rounded-md bg-orange-500 text-white"
              @click="${() => state.connectionHelpDialogOpen = true}"
            >
              Help me!?
            </button>
          </div>
        ...
        if (state.selectedAction === `openPluginsPage`) {
          window.open(
            `${state.auth.config.baseUrl}/web/index.html#!/addplugin.html?name=Playback%20Reporting&guid=5c53438191a343cb907a35aa02eb9d2c`,
            `_blank`,
          );
    ...
    const viewServer = html`
      <div class="p-4">
        ...
        <div
          class="flex flex-col gap-4 text-lg font-medium leading-6 text-gray-500 dark:text-gray-400 mt-10 w-full mx-auto text-balance text-center px-2"
        >
          <p class="">
        ...
        <form class="flex flex-col gap-4 mt-10 w-5/6 mx-auto" @submit="${(e) =>
          e.preventDefault()}">
          <label class="flex flex-col gap-1">
            ...
            <input
              ...
              @keydown="${(e) =>
                e?.key?.toLowerCase() === `enter` && connect()}"
            >
          </label>
        </form>
      ...
      <div class="p-4">
        ...
        <div class="mt-6 w-5/6 mx-auto">
          ...
          <ul class="flex flex-col gap-4 mt-2">
            <div
              class="flex flex-row flex-wrap justify-center gap-2 max-h-[14rem] overflow-x-hidden"
            >
              ${() =>
                state.server.users.map((user) =>
                  html`
                    <li
                      ...
                      <img
                        ...
                        src="${() =>
                          user.PrimaryImageTag
                            ? `${state.auth.config.baseUrl}/Users/${user.Id}/Images/Primary?tag=${user.PrimaryImageTag}`
                            : `/media/ArtistPlaceholder.png`}"
                      >
                      <span class="text-lg text-wrap text-center font-semibold text-gray-700"
                      >${user.Name}</span>
                    </li>
                  `
                )}
            </div>
            <span class="w-full text-center text-gray-600 font-semibold text-xs -my-1"
            >or</span>
            <div class="flex flex-col gap-2">
              ...
              >
                <span class="text-base text-center w-full font-regular text-gray-900"
                >Manually enter username</span>
              </li>
              <span
                class="w-full text-center text-gray-600 font-semibold text-xs -mt-1.5 -mb-1"
              >or</span>
              <li
                class="flex flex-row gap-4 items-center bg-white shadow-sm rounded-xl p-2"
                @click="${() => {
    ...
    function inspectPlaybackReportingSetup(playbackReportingSetup, nextScreen) {
      ...
      if (!playbackReportingSetup.installed) {
        ...
        if (window.helper && state.auth.config.user.isAdmin) {
          inspection.action = html`
            <button
              ...
              @click="${async () => {
                try {
                  await window.helper.enablePlaybackReportingPlugin(
        ...
        parseInt(playbackReportingSetup.version) < 13
      ) {
        inspection.valid = false;
        inspection.issue =
        ...
        inspection.valid = false;
        inspection.issue =
          `The Playback Reporting plugin is installed, but the retention interval is short (${playbackReportingSetup.settings?.retentionInterval} months).`;
        if (window.helper && state.auth.config.user.isAdmin) {
          inspection.action = html`
            <button
              ...
              @click="${async () => {
                try {
                  ...
                  await window.helper.updatePlaybackReportingSettings(
                    newSettings,
                  );
                  checkPlaybackReportingSetup(nextScreen);
                } catch (err) {
        ...
        playbackReportingSetup.ignoredUsers.some((user) =>
          user.id === state.auth?.config?.user?.id
        )
      ) {
        inspection.valid = false;
        inspection.issue =
        ...
        inspection.action = html`
          ...
          >Open Playback Reporting Settings!</a>
          <button
            class="px-2 py-1 rounded-lg text-sm font-semibold border-2 border-orange-400 hover:bg-orange-500 dark:border-orange-500 dark:hover:bg-orange-600 text-orange-500 hover:text-white"
            @click="${() => checkPlaybackReportingSetup(nextScreen)}"
          >
            Check Again
          </button>
        `;
    ...
    async function checkPlaybackReportingSetup(nextScreen) {
      ...
      if (
        await checkIfOfflinePlaybackImportAvailable(state.auth) &&
        nextScreen !== `revisit`
      ) {
      ...
      try {
        const playbackReportingSetup = await window.helper
          .checkIfPlaybackReportingInstalled();
        console.log(`playbackReportingSetup:`, playbackReportingSetup);

        const inspection = inspectPlaybackReportingSetup(
          playbackReportingSetup,
          nextScreen,
        );
        ...
        console.error(
          `Failed to check the playback reporting setup, continuing without it:`,
          err,
        );
    ...
    const viewPlaybackReportingIssues = html`
      <div class="p-4">
        ...
        <div
          class="w-full px-6 flex flex-col gap-3 items-center text-center mt-10 text-gray-700 dark:text-gray-300 text-balance text-center"
        >
          <p
            class="w-full text-lg text-balance font-semibold text-red-500 dark:text-red-400"
          >
    ...
    const viewLogin = html`
      <div class="p-4">
        ...
        <div
          class="flex flex-col gap-4 text-lg font-medium leading-6 text-gray-500 dark:text-gray-400 mt-10 w-5/6 mx-auto text-balance text-center"
        >
          <p class="">Almost there!</p>
          <p class="">
            Please enter your credentials to log into your server.<br>Your password
            will only be sent to your server.
          </p>
        </div>
        ...
        <form class="flex flex-col gap-4 mt-10 w-5/6 mx-auto" @submit="${(e) =>
          e.preventDefault()}">
          ${() =>
            [`password`, `full`].includes(state.server.loginType)
              ? html`
                <label class="flex flex-col gap-1">
                  <span class="text-sm ml-1.5 font-medium">Username</span>
                  <input
                    class="${() =>
                      `px-3 py-1.5 dark:bg-[#101010] dark:text-gray-200 rounded-lg focus:ring-1 focus:ring-[#00A4DC] focus:outline-none ${
                        state.server.selectedUser
                    ...
                    value="${() =>
                      state.server.selectedUser
                        ? state.server.selectedUser.Name
                        : ``}"
                    @keydown="${(e) =>
              ...
              : html`
                <label class="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    @keydown="${(e) =>
                      e?.key?.toLowerCase() === `enter` && loginAuthToken()}"
                  >
                </label>
              `}
        </form>
    ...
    const viewImportReportForViewing = html`
      <div class="p-4">
        ...
        <div class="w-full flex flex-col items-center text-center mt-12">
          ${() =>
            html`
              <label for="import-file" class="${() =>
                `px-7 py-3 rounded-2xl text-[1.4rem] bg-[#00A4DC] hover:bg-[#0085B2] text-white font-semibold flex flex-row gap-4 items-center mx-auto ${
                  state.importingExistingReport ? `saturation-50` : ``
                }`}">Import Report</label>
              <input
                type="file"
                id="import-file"
                ...
                @change="${async (e) => {
                  ...
                  try {
                    ...
                    input.disabled = true;
                    state.rewindReport = await importRewindReport(
                      e.target.files[0],
                    );
                    state.auth.config.serverInfo =
                      state.rewindReport.jellyfinRewindReport.server;
                    console.log(
                      `state.auth.serverInfo:`,
                      state.auth.serverInfo,
                    );
                    console.log(`state.rewindReport:`, state.rewindReport);
              ...
              ${() =>
                state.importingExistingReport
                  ...
                  : html`
                    ${() =>
                      JSON.parse(import.meta.env.VITE_SHOW_PLACEHOLDER)
                        ? html`
                          <button
                            class="px-2 py-1 rounded-lg text-sm border-[#00A4DC] border-2 hover:bg-[#0085B2] font-medium text-gray-700 dark:text-gray-200 mt-8 flex flex-row gap-4 items-center mx-auto hover:text-white"
    ...
    const viewLaunchExistingReport = html`
      <div class="p-4">
        ...
        >
          <span>${() =>
            state.rewindGenerating ? `Generating...` : `Launch Rewind!`}</span>
          ${() =>
            !state.rewindGenerating
              ? html`
                <svg
                  xmlns="http://www.w3.org/2000/svg"
    ...
    const viewImportOfflinePlayback = html`
      <div class="p-4">
        ...
        <div class="w-full flex flex-col items-center text-center mt-12">
          <label for="import-file" class="${() =>
            `px-7 py-3 rounded-2xl text-[1.4rem] bg-[#00A4DC] hover:bg-[#0085B2] text-white font-semibold flex flex-row gap-4 items-center mx-auto ${
              state.importingOfflinePlayback ? `saturation-50` : ``
            }`}"
          ...
          <input
            ...
            @change="${async (e) => {
              ...
              try {
                ...
                input.disabled = true;
                state.offlinePlayback = await importOfflinePlayback(
                  e.target.files[0],
                );
                console.log(`state.offlinePlayback:`, state.offlinePlayback);
                console.log(
                  `missing playDurations:`,
                  state.offlinePlayback.filter((x) => !x.playDuration),
                );

                // import plays to server
                await uploadOfflinePlaybackBatched(
                  state.offlinePlayback,
                  state.auth,
                );
          ...
          ${() =>
            state.importingOfflinePlayback
              ? html`
                <p
                  class="mt-8 px-10 text-xl text-balance font-semibold text-gray-600 dark:text-gray-300"
                >
          ...
          ${() =>
            !state.oldReport
              ? html`
                <label for="import-file" class="${() =>
                  `px-7 py-3 rounded-2xl text-[1.4rem] bg-[#00A4DC] hover:bg-[#0085B2] text-white font-semibold flex flex-row gap-4 items-center mx-auto ${
                    state.importingLastYearsReport ? `saturation-50` : ``
                ...
                <input
                  ...
                  @change="${async (e) => {
                    ...
                    try {
                      ...
                      input.disabled = true;
                      state.oldReport = await importRewindReport(
                        e.target.files[0],
                      );
                      ...
                      console.error(
                        `Error while importing rewind report:`,
                        err,
                      );
                    }
                    input.disabled = false;
                ...
                ${() =>
                  state.importingLastYearsReport
                    ? html`
                      <p
                        class="mt-8 px-10 text-xl text-balance font-semibold text-gray-600 dark:text-gray-300"
                      >
    ...
    const progressBar = html`
      <div class="w-5/6 mx-auto mt-10 flex h-8 flex-row gap-4 justify-left">
        <img class="${() =>
          `inline h-full ${
            state.progress < 1 ? `animate-spin` : ``
          }`}" src="/media/jellyfin-rewind-icon.svg" />
        <div
          class="w-full flex flex-row gap-2 items-center bg-white dark:bg-[#101010] rounded-full"
        >
          <div
    ...
    const viewLoad = html`
      <div class="p-4">
        ...
        >
          ...
          <p class="">
            Please be patient, and if nothing happens for more than 30s, reach out to
            me via <a
              class="text-[#00A4DC] hover:text-[#0085B2]"
              href="https://github.com/Chaphasilor/jellyfin-rewind/issues"
              target="_blank"
            >GitHub</a> so that I can look into it :)
          </p>
        </div>
          ...
          state.rewindReport
            ? html`
              <button
                ...
                @click="${() => launchRewind()}"
                disabled="${() =>
                  !state.rewindReport || state.rewindGenerating}"
              >
                <span>${() =>
                  state.rewindGenerating
                    ? `Generating...`
      ...
      <div class="p-4">
        ...
        >
          ...
          <p class="">${() =>
            !state.staleReport
              ? html`
                <span>Your Rewind Report is still saved. You can view it any time you
                  like.</span>
              `
              : html`
                <span class="font-bold text-orange-500"
                >The stored Rewind report is stale. Please re-generate it for the best
                  experience.</span>
              `}</p>
        </div>
        ...
        >
          ...
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="w-5 h-5 stroke-[2.5] icon icon-tabler icon-tabler-refresh"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            stroke-width="2"
    ...
    const viewRewindGenerationError = html`
      <div class="p-4">
        ...
        <div
          class="flex flex-col gap-4 text-lg font-medium leading-6 text-gray-500 dark:text-gray-400 mt-10 w-5/6 mx-auto"
        >
          <p class="">Sorry, we couldn't generate your Rewind Report.</p>
          <p class="">Please try again later.</p>
          <p class="">
            If you keep seeing this message, please reach out to me on <a
              class="text-[#00A4DC] hover:text-[#0085B2]"
              href="https://github.com/Chaphasilor/jellyfin-rewind/issues"
              target="_blank"
            >GitHub</a> or <a
              ...
              target="_blank"
            >Reddit</a> so that I can try to resolve the issue!
          </p>
        </div>
