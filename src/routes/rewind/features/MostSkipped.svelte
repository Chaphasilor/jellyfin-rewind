...
<div class="text-center">
  ...
  {#if extraFeatures().mostSkippedTracks}
    <ol id="most-skipped-tracks-main-feature" class="flex flex-col gap-2 p-6">
      {#each $lightRewindReport.jellyfinRewindReport.tracks?.[`mostSkipped`]?.slice(0, 5) as track, index (track.id)}
        <li
            ...
            >
              <div>
                <span class="font-semibold text-black dark:text-white"
                  >{showAsNumber(track.skips.total)}</span
                >
                skips
              </div>
              <svg
...
{#if extraFeatures().mostSkippedTracks}
  ...
  >
    {#each $lightRewindReport.jellyfinRewindReport.tracks?.[`mostSkipped`]?.slice(5, 20) as track, index (track.id)}
