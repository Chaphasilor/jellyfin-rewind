...
{#if monthWithMostDuration == monthWithMostPlays} 
    <h4>
        ...
        you had the most playbacks and playtime
        with <b>{duration[mostDuration]} minutes</b> during
...
{:else}
    ...
    <h5>
        ...
        <b>{monthWithLeastPlays}</b> holds the record for least
        plays. 
    </h5>
{/if}
...
<h5>
    <b>{monthWithLeastSkips}</b> made you listen more actively than
    any other month with just <b>{skips[leastSkips]} skips</b> while
    <b>{monthWithMostSkips}</b> was the month with the most skips at
    a total of <b>{skips[mostSkips]}</b>.
</h5>
